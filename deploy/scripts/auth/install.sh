...
for SITE_ROLE in $SITE_ROLES; do
  kcadm create clients/$USER_CLIENT_ID/roles -r $KC_REALM -s name=APP_$SITE_ROLE
done
...
kcadm update clients/$USER_CLIENT_ID -r $KC_REALM -s "secret=$(cat $KC_USER_CLIENT_SECRET_FILE)"
