...
func TestIntegrationQuotes(t *testing.T) {
	...
	t.Run("multiple users can request the same slot", func(t *testing.T) {
		var err error
		quoteIds := make([]string, 3)
		for i, member := range []*TestUser{member1, member2, member3} {
			...
			} else if !slices.Contains(quoteIds, member.Quote.Id) {
				quoteIds[i] = member.Quote.Id
			}
		}

		for i, id := range quoteIds {
			if !util.IsUUID(id) {
				t.Errorf("quote id #%d is not a uuid, got: %s", i+1, id)
			}
		}
	})
