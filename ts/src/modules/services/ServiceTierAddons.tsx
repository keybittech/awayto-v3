...
export function ServiceTierAddons({ service }: IComponent): React.JSX.Element {
  ...
  const tierGridProps = useGrid({
    ...
    columns: [
      ...
      ...serviceTiers.map<GridColDef<{ tiers: string[] }>>(st => ({
        ...
        headerName: st.name,
        renderCell: params => st.name && params.row.tiers.includes(st.name) ? <Avatar sx={{ width: 24, height: 24, backgroundColor: 'white' }}><CheckIcon sx={classes.green} /></Avatar> : '--',
        flex: 1
      }))
    ]
  });
